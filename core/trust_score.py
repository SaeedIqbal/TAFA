...
def compute_data_quality_score(embeddings, labels, server_stats):
    """Compute Q_m: Sharpness, SNR, Label Consistency."""
    # Sharpness: variance of Laplacian
    sharpness = np.mean([variance(ndimage.laplace(e.cpu().numpy())) for e in embeddings])
    
    # SNR: defect vs background (assume labels indicate defect regions)
    ...
    # Label Consistency: assume 5% subset has double annotations
    label_consistency = 0.95  # Simulated; replace with Fleiss' Kappa in real setup
...
def compute_decoder_reliability_score(model, val_loader, device):
    """Compute R_m: Reconstruction loss + Synthetic F1."""
    model.eval()
    recon_losses = []
    synthetic_data = []
    synthetic_labels = []
...
def compute_global_consistency_score(encoder, val_loader, device, latent_dim=128):
    """Compute C_m: exp(-KL(q_phi(z) || N(0,I)))."""
    encoder.eval()
    zs = []
    with torch.no_grad():
